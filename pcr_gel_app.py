...
class PCRGelApp:
    ...
    def __init__(self, root):
        ...
        self.samples = []  # Lista de (nombre, tamaño) en orden de carga
    ...
    def update_gel(self):
        ...
        samples = self.samples.copy()  # Mantener el orden original
        ...
        # Calcular migración para muestras (en el orden que fueron añadidas)
        samples_data = []
        ...
        # Dibujar carril de ladder (siempre en la posición 0)
        draw_lane(self.ax, 0, ladder_data, f"Ladder\n{self.selected_ladder.get()}", 
                  is_ladder=True, show_labels=True)
        
        # Dibujar carriles para muestras en el orden en que fueron añadidas
        if samples_data:
            # Usar el orden original de samples_data (que mantiene el orden de adición)
            for i, sample in enumerate(samples_data):
                x_pos = (i + 1) * 2.5  # Posición después del ladder
                draw_lane(self.ax, x_pos, [sample], sample['name'], 
                          is_ladder=False, show_labels=True)
        
        # Configurar el gráfico
        max_lanes = max(1, len(samples_data))
        # Ajustar el límite x para que quepan todos los carriles
        x_limit = max(15, (max_lanes + 1) * 2.5 + 1)
        self.ax.set_xlim(-2, x_limit)
